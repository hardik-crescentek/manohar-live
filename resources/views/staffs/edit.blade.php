        ...
        <div class="col-lg-12 col-md-12">
            <div class="card custom-card">
                <div class="card-body">
                    ...
                    <form class="parsley-validate" method="post" action="{{ route('staffs.update', $staff->id) }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="custom-radio-div">
                                    <label class="rdiobox d-inline-block mr-4" for="salaried">
                                        <input type="radio" class="type" name="type" value="1" id="salaried"
                                            {{ $staff->type == 1 ? 'checked' : '' }}> <span>Salaried</span>
                                    </label>
                                    <label class="rdiobox d-inline-block" for="on-demand">
                                        <input type="radio" class="type" name="type" value="2" id="on-demand"
                                            {{ $staff->type == 2 ? 'checked' : '' }}> <span>On demand</span>
                                    </label>
                                </div>
                            </div>
                            <div class="col-md-6 {{ $staff->type != 1 ? 'd-none' : '' }} salaried-container">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" name="salary" type="text"
                                        onkeypress="return onlyNumbers(event)" value="{{ old('salary', $staff->salary) }}">
                                </div>
                            </div>
                            <div class="col-md-6 {{ $staff->type != 2 ? 'd-none' : '' }} demand-container">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" name="rate_per_day" type="text"
                                        ...
                                        value="{{ old('rate_per_day', $staff->rate_per_day) }}">
                                </div>

                                <div class="form-group">
                                    <label class="">Labour Number</label>
                                    <input class="form-control" name="labour_number" type="text"
                                        onkeypress="return onlyNumbers(event)"
                                        value="{{ old('labour_number', $staff->labour_number) }}">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#joining_date').datepicker({
                ...
                '{{ $staff->joining_date ? date('Y-m-d', strtotime($staff->joining_date)) : date('Y-m-d') }}'
                ));

            // Initialize Resign Date
            $('#resign_date').datepicker({
                ...
                '{{ $staff->resign_date ? date('Y-m-d', strtotime($staff->resign_date)) : date('Y-m-d') }}'
                ));

            // Show/hide salaried or demand container
            $('.type').change(function() {
                $('.salaried-container').toggleClass('d-none', $('input[name="type"]:checked').val() != 1);
                $('.demand-container').toggleClass('d-none', $('input[name="type"]:checked').val() != 2);
            });
        });
    </script>
@endsection
