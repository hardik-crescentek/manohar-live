    ...
    <tbody>
        ...
        <tr class="tr-{{$item->id}}">
            ...
            <td>
                ...
                <a href="{{ route('staffs.edit', $item->id) }}" data-toggle="tooltip" title="Edit"> <i class="fa fa-pen text-primary mr-2"></i> </a>
                @endif
                @if(Auth::user()->hasrole('super-admin') || Auth::user()->can('staffs-delete'))
                <a href="javascript:;" class="delete-staffs" data-id="{{ $item->id }}" data-route="{{ route('staffs.destroy', $item->id) }}" data-toggle="tooltip" title="Delete"> <i class="fa fa-trash text-danger"></i> </a>
                @endif
            </td>
        </tr>
        @endforeach
